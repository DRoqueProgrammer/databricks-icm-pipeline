...
Rows with the same `state_id` and `year` are duplicates. The first occurrence (by `ingestion_date`) is kept; the rest are quarantined with reason `duplicate`.
...
# In the Silver notebook:
fk_rules = {
    "state_id": ("dbfs:/Volumes/ibge/reference/states_clean", "state_id not in reference table"),
}
