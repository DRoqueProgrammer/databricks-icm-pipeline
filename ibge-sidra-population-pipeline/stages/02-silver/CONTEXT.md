...
|--------|--------------|---------------|-----|
| Previous stage | `../01-bronze/output/` | Most recent `ibge-sidra-population-bronze-notebook.py` | The Bronze notebook -- defines the Bronze Delta path to read from |
...
5. Author the Silver notebook in `output/ibge-sidra-population-silver-notebook.py`. The notebook must:
   - Read from the Bronze Delta path
   - Apply type coercion to the target schema from conventions.md
   - Deduplicate using `state_id`, `year` (keep first by `ingestion_date`)
   - Filter rows where required columns (`state_id`, `year`) are null, sending them to quarantine
   - Apply value range checks (e.g. `year BETWEEN 1900 AND 2100`)
   - Apply FK checks if configured
   - Write the surviving rows to `dbfs:/Volumes/ibge/ibge_sidra_population_silver/cleansed/ibge-sidra-population_clean` as Delta
   - Write the quarantined rows to `dbfs:/Volumes/ibge/ibge_sidra_population_silver/quarantine/quarantine_ibge-sidra-population_{{silver_table}}` with `_dq_failure_reason`, `_dq_failure_value`, `_dq_check_time` columns
   - Write a DQ report to `output/ibge-sidra-population-dq-report.md` (counts, top failure reasons, sample bad rows)
...
| Required columns not null | 0 nulls in `state_id` and `year` in Silver |
| Quarantine table exists | A Delta table exists at `dbfs:/Volumes/ibge/ibge_sidra_population_silver/quarantine/quarantine_ibge-sidra-population_*` if any rows were quarantined |
| DQ report exists | `output/ibge-sidra-population-dq-report.md` is non-empty and contains row counts |
...
|----------|----------|--------|
| Silver notebook | `output/ibge-sidra-population-silver-notebook.py` | Python notebook |
| DQ report | `output/ibge-sidra-population-dq-report.md` | Markdown summary: row counts, top failure reasons, sample bad rows |
| Silver Delta table | `dbfs:/Volumes/ibge/ibge_sidra_population_silver/cleansed/ibge-sidra-population_clean` | Delta table on Databricks (read-only from here) |
| Quarantine Delta table | `dbfs:/Volumes/ibge/ibge_sidra_population_silver/quarantine/quarantine_ibge-sidra-population_clean` | Delta table with `_dq_failure_reason`, `_dq_failure_value`, `_dq_check_time` |
