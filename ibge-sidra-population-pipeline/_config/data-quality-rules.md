...
Rows that fail Silver checks land in `dbfs:/Volumes/ibge/ibge_sidra_population_silver/quarantine` as a Delta table named:

```
ibge.ibge_sidra_population_silver.quarantine_{dataset_slug}_{silver_table_name}
```
...
```yaml
dataset:
  slug: "ibge-sidra-population"
  primary_keys: ["state_id", "year"]   # composite if needed
  ...
  foreign_keys:
    "state_id": "dbfs:/Volumes/ibge/reference/states_clean"
```
