...
|-------|--------|-------------|
| 01-bronze | `stages/01-bronze/output/ibge-sidra-population-bronze-notebook.py` | Stage 02 reads it as the source of truth for what landed in Bronze |
| 02-silver | `stages/02-silver/output/ibge-sidra-population-silver-notebook.py` + `ibge-sidra-population-dq-report.md` | Stage 03 reads the notebook; DQ report goes to Stage 04 |
| 03-gold | `stages/03-gold/output/ibge-sidra-population-gold-notebook.py` | Stage 04 reads the Gold schema and row counts |
| 04-report | `stages/04-report/output/ibge-sidra-population-report.md` | Human reads, edits, shares with stakeholders |