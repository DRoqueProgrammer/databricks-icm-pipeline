...
```
Pipeline Status: databricks-pipeline (dataset={{DATASET_SLUG}})
...
Then run `setup`. The questionnaire fills in `{{DATASET_SLUG}}`, `{{CATALOG_NAME}}`, volume paths, and cluster config. Every run on a new dataset starts from a fresh copy.