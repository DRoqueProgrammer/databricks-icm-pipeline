...
# Read Silver Delta path (extracted from ../02-silver/output/{{DATASET_SLUG}}-silver-notebook.py)
...
for name, df in aggregates.items():
    gold_path = f"{{GOLD_VOLUME_PATH}}/{name}"
    ...
    # Add column comments
    spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {{CATALOG_NAME}}.{{GOLD_SCHEMA}}.{name}
        USING DELTA
        LOCATION '{gold_path}'
    """)
    ...
    for col in df.columns:
        ...
        spark.sql(f"""
            ALTER TABLE {{CATALOG_NAME}}.{{GOLD_SCHEMA}}.{name}
    ...
    if zorder_cols:
        spark.sql(f"OPTIMIZE {{CATALOG_NAME}}.{{GOLD_SCHEMA}}.{name} ZORDER BY ({', '.join(zorder_cols)})")
...
states_df = spark.read.format("delta").load("dbfs:/Volumes/{{CATALOG_NAME}}/reference/states_clean")
