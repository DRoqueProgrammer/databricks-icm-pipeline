...
|--------|--------------|---------------|-----|
| Previous stage | `../02-silver/output/` | Most recent `{{DATASET_SLUG}}-silver-notebook.py` and `{{DATASET_SLUG}}-dq-report.md` | Silver schema and DQ caveats |
...
2. Read the DQ report (`{{DATASET_SLUG}}-dq-report.md`). Note the row counts and quarantine reasons -- if many rows were quarantined for a specific reason, you may want to flag that in the Gold report.
...
5. Author the Gold notebook in `output/{{DATASET_SLUG}}-gold-notebook.py`. The notebook must:
   ...
   - Write each aggregate to `{{GOLD_VOLUME_PATH}}/{aggregate_name}` as Delta
...
|----------|----------|--------|
| Gold notebook | `output/{{DATASET_SLUG}}-gold-notebook.py` | Python notebook with one cell per aggregate |
| Gold Delta tables | `{{GOLD_VOLUME_PATH}}/{aggregate_name}` | One Delta table per confirmed aggregate |
