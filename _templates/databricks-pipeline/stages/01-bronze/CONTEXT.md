...
4. Author the Bronze notebook in `output/{{DATASET_SLUG}}-bronze-notebook.py` using the template from `references/notebook-template.py`. The notebook must:
   ...
   - Partition by `{{PARTITION_BY_DEFAULT}}` (default `ingestion_date`)
   - Write to `{{BRONZE_VOLUME_PATH}}/{{DATASET_SLUG}}` as Delta, mode `append`
...
| Partition column present | The DataFrame has the column specified in `{{PARTITION_BY_DEFAULT}}` |
...
| Source lineage recorded | A separate manifest row written to `{{BRONZE_VOLUME_PATH}}/_manifest/` with source path, row count, batch_id, ingestion_date |
...
|----------|----------|--------|
| Bronze notebook | `output/{{DATASET_SLUG}}-bronze-notebook.py` | Python notebook (.py) with cell delimiters (`# COMMAND ----------`) |
| Optional manifest | `output/{{DATASET_SLUG}}-bronze-manifest.json` | One JSON object per ingested file: source path, row count, batch_id, timestamp |
